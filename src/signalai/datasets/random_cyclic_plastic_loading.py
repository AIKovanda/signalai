...
class RandomCyclicPlasticLoadingParamsV1:
    ...
    def unscale_params(self, scaled_params: np.ndarray):
        ...
        return {
            ...
            'c': np.exp(np.array([
                np.log(scaled_params[3]) * 0.64220986 + 8.07521718,
                np.log(scaled_params[4]) * 0.70230589 + 6.66012441,
                np.log(scaled_params[5]) * 0.82282555 + 5.75443793,
                np.log(scaled_params[6]) * 0.71284407 + 4.83443022,
            ])),
    ...
    def generate(cls, depspc_r=None, depspc_r_number: int = None):
        ...
        a /= np.sum(a)
        return RandomCyclicPlasticLoadingParamsV1(params={
...
def random_cyclic_plastic_loading(rcpl_params: RandomCyclicPlasticLoadingParamsV1, steps: int) -> TimeSeries:
...
class RandomCyclicPlasticLoadingGen(TimeSeriesGen):
    ...
    def _getitem(self, _: int) -> TimeSeries:
        model_params = RandomCyclicPlasticLoadingParamsV1.generate(
